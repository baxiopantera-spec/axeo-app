...
const manuals = JSON.parse(
  await fs.readFile(path.join(__dirname, "manuals.json"), "utf8")
);
const catalog = JSON.parse(
  await fs.readFile(path.join(__dirname, "catalog.json"), "utf8")
);

if(!PASSWORD_SECRET){
  console.warn("WARNING: PASSWORD_SECRET is not set.");
}
if(!BOT_TOKEN){
  console.warn("WARNING: BOT_TOKEN is not set; Telegram bot is disabled.");
}
if(ALLOWED_TELEGRAM_IDS.size === 0){
  console.warn("WARNING: ALLOWED_TELEGRAM_IDS is empty; no Telegram user can receive passwords.");
}
...
function dailyPassword(manualKey, dateKey = rotationKey()){
  ...
  const digest=crypto.createHmac("sha256",PASSWORD_SECRET)
    .update(`${dateKey}|${manualKey}`)
    .digest();
  let out="";
...
function catalogLabel(item){return `${item.display || item.name} · ${item.sub || "Мануал"}`;}

function keyboard(){
  const rows=[];
  for(let i=0;i<catalog.length;i+=2){
    rows.push(catalog.slice(i,i+2).map((item,j)=>({text:catalogLabel(item).slice(0,64),callback_data:`p:${i+j}`})));
  }
...
async function handleUpdate(update){
  if(update.callback_query){
    ...
    const idx=Number(String(q.data||"").replace("p:",""));
    const item=catalog[idx];
    if(item) await sendPassword(q.message.chat.id,item);
    return;
  }
  ...
  if(msg.chat?.type !== "private"){
    if(/^\/(start|pass|passwords)/i.test(text)){
  ...
  if(/^\/(start|pass|passwords|help)/i.test(text)){
    await telegram("sendMessage",{chat_id:msg.chat.id,text:"Выберите мануал, для которого нужен актуальный пароль.\n\nПароль действует до следующего ежедневного обновления.",reply_markup:keyboard()});
  }
}
...
async function botLoop(){
  ...
  try{
    ...
    let offset=0;
    while(true){
      const updates=await telegram("getUpdates",{offset,timeout:45,allowed_updates:["message","callback_query"]});
      for(const update of updates){
        offset=update.update_id+1;
        try{await handleUpdate(update);}catch(err){console.error("Bot update error",err);}
      }
    }
  }catch(err){
...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method === "GET" && url.pathname === "/health"){
    json(res,200,{ok:true,rotationDate:rotationKey(),nextRotationAt:nextRotationAt().toISOString()});
    return;
  }
  ...
  if(req.method === "POST" && url.pathname === "/api/manual/verify"){
    ...
    }catch(err){json(res,400,{ok:false,error:"BAD_REQUEST"});}
    return;
  }
