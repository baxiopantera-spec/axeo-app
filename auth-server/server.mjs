...
for(const item of catalog){
  ...
  directions.get(parsed.direction).push({...item, __leaf:parsed.leaf, __catalogIndex:catalog.indexOf(item)});
}
...
function bankKeyboard(bankIndex){
  ...
  for(const [direction, items] of entry.directions.entries()){
    ...
    row.push({
      ...
      callback_data: isSingle ? `p:${item.__catalogIndex}` : `d:${bankIndex}:${dIndex}`
    });
    dIndex++;
...
function directionKeyboard(bankIndex, directionIndex){
  ...
  for(let i = 0; i < items.length; i += 2){
    ...
    for(let j = i; j < Math.min(i + 2, items.length); j++){
      const item = items[j];
      row.push({ text: manualButtonLabel(item), callback_data: `p:${item.__catalogIndex}` });
    }
    rows.push(row);
  }
  rows.push([
...
async function handleUpdate(update){
  if(update.callback_query){
    ...
    if(data.startsWith("p:")){
      ...
      if(item && item.key && manuals[item.key]) await sendPassword(chatId, item);
      return;
    }
    return;
  }
